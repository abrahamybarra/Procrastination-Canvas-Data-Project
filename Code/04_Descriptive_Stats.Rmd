...
```{r}
######## Histogram plotting function ############
plot_histogram <- function(df, xvar, binwidth) {
  p <- ggplot(df) +
...
plot_histogram(df = studentSummary_21A, xvar = "OVERALL_COURSE_GRADE", binwidth = 5)
plot_histogram(df = studentSummary_21A, xvar = "FINAL_EXAM_SCORE", binwidth = 5)
plot_histogram(df = studentSummary_21A, xvar = "PCT_ON_TIME", binwidth = .05)
plot_histogram(df = studentSummary_21A, xvar = "MEAN_HOUR_DIFF", binwidth = .05)
plot_histogram(df = assignments_21A, xvar = "HOUR_DIFF", binwidth = .05)
