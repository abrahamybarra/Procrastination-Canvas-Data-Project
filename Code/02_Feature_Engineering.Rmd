...
assignments_21A_submitted <- assignments_21A_submitted %>%
  mutate(HOUR_DIFF = difftime(DUE_DATE, SUBMITTED_AT, units = "hours") %>% as.numeric(), .after = SUBMITTED_AT_TIME)
...
assignments_21A <- assignments_21A %>%
  left_join(assignments_21A_submitted %>% 
              select(c(UID, HOUR_DIFF, YJ_NEG_2, YJ_NEG_1, YJ_0, YJ_POS_1, YJ_POS_2)), 
            by = "UID") %>%
  relocate(HOUR_DIFF:YJ_POS_2, .after = SUBMITTED_AT_TIME) %>%
  mutate(LOG_HOUR_DIFF = log(HOUR_DIFF), .after = HOUR_DIFF)
```
