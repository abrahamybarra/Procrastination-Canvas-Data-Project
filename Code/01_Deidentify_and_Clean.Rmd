...
# TODO: Figure out the issue with date format for SUBMITTED_AT
# (TEMP FIX) remove assignments with no submission
assignments_21 <- assignments_21 %>%
...
# Compute time difference in hours and store result in new column. Negative means early, positive means late
# assignments_21 <- assignments_21 %>% 
#   mutate(hour_diff = ifelse(SUBMITTED_AT == "", NA, 
#                             difftime(as.POSIXct(SUBMITTED_AT), as.POSIXct(DUE_DATE), units = "hours") %>% as.numeric()), 
