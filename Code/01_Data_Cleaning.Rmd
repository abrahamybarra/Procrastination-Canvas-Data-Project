...
## Performs cleaning on the Canvas assignment submission data, including removal of certain assignments, filtering out unnecessary columns, and separation of the data by class section.  
...
```{r}
# Note: No longer necessary (OIT fixed)
...
f21_section = get_section(paste0(here(),"/raw_data/Additional Canvas Datasets/F21/gradebook.csv"), 'F21') # N = 536
f22_section = get_section(paste0(here(),"/raw_data/Additional Canvas Datasets/F22/gradebook.csv"), 'F22') # N = 192
...
assignments <- read.csv(paste0(here(),"/raw_data/psyc_students_report_split_v2.csv"), 
                        header = TRUE, numerals = "no.loss") # N = 536

# Dataset with student ID's
sid_key <- read.csv(paste0(here(),"/raw_data/sid_key_v2.csv"), header = TRUE)

# Additional Canvas Datasets (F21 only)
# # Recover overall course grades for Ashley section
# studentSummary_21A <- read.csv(paste0(here(),"/raw_data/F21/studentSummary.csv"), header = TRUE) %>%
#   select(c(STUDENT_ID = SIS.Id, Overall.course.grade)) %>%
#   inner_join(sid_key, by = "STUDENT_ID") %>% 
#   select(-STUDENT_ID)
...
assignments <- assignments %>%
  select(ANON_ID, OVERALL_COURSE_GRADE = CURRENT_SCORE, ASSIGNMENT_ID, ASSIGNMENT_NAME, SUBMITTED_FLAG, SCORE, POINTS_POSSIBLE,
...
# ADD Student ID (DEIDENTIFY LATER!), rearrange columns.
assignments <- assignments %>%
...
# Separate by year, validate section with gradebook
assignments_21 <- assignments %>% # (N = 345)
...
# TODO: There are 5 total such assignments for F21-002 (Jake's section) that will need to be removed.
...
assignment_list_21 = unique(assignments_21$ASSIGNMENT_NAME)
num_assignments = length(assignment_list_21) # total number of assignments (N = 83)
assignment_list_21
...
## 6) Deidentify data pulled directly from Canvas (F21) (Not applicable to OIT datasets)
...
## 7) Split dataset by section

```{r}
assignments_21A <- assignments_21 %>%
  filter(CLASS_SECTION_CD == 1)

assignments_21J <- assignments_21 %>%
  filter(CLASS_SECTION_CD == 2)
```
...
# write.csv(assignments,paste0(here(),"/clean_data/assignments.csv"),row.names = FALSE)
write.csv(assignments_21A,paste0(here(),"/clean_data/F21_Ashley/assignments.csv"),row.names = FALSE)
write.csv(assignments_21J,paste0(here(),"/clean_data/F21_Jake/assignments.csv"),row.names = FALSE)
write.csv(assignments_22,paste0(here(),"/clean_data/F22_Abraham/assignments.csv"),row.names = FALSE)
```
